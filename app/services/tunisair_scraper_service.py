...
class TunisairScraper:
    ...
    def _get_exchange_rate(self) -> float:
        if not self.api_key_provided:
            logger.warning(f"API Key not found. Using fallback rate: 1 TND = {self.fallback_eur_rate:.4f} EUR")
    ...
    def _extract_prices(self, html: str, is_eur_native: bool, conversion_rate: float) -> List[Dict[str, Any]]:
        ...
        for td in soup.find_all("td", class_="available"):
            date_str, price_div = td.get("data-departure"), td.find("div", class_="val_price_offre")
            ...
            try:
                ...
                if is_eur_native and "EUR" in price_text:
                    ...
                    flight_data = {"price": price_tnd, "priceEur": round(price_tnd * conversion_rate, 2)}
                else:
                    continue
                flight_data["departureDate"] = departure_date.isoformat()
                found_flights.append(flight_data)
            except (ValueError, TypeError) as e:
                logger.warning(f"Could not parse record '{date_str}' | '{price_text}'. Error: {e}")
    ...
    def _scrape_route(self, dep_code: str, arr_code: str, is_eur_native: bool, conversion_rate: float = 1.0) -> List[Dict[str, Any]]:
        ...
        route_flights = []
        today = date.today()
        search_dates = [today.strftime("%Y-%m-%d")] + [(today + relativedelta(months=i)).strftime("%Y-%m-01") for i in range(1, MONTHS_TO_SEARCH)]
        for search_date in search_dates:
            params = {"date": search_date, "from": dep_code, "to": arr_code, "tripDuration": DEFAULT_TRIP_DURATION, "tripType": DEFAULT_TRIP_TYPE}
            html_view = None
            for attempt in range(REQUEST_RETRIES):
                try:
                    ...
                    logger.warning(f"Attempt {attempt + 1}/{REQUEST_RETRIES} failed for {dep_code}->{arr_code} on {search_date}: {e}")
                    if attempt < REQUEST_RETRIES - 1:
                        time.sleep(1)
            if html_view:
                for flight in self._extract_prices(html_view, is_eur_native, conversion_rate):
                    flight.update({"departureAirportCode": dep_code, "arrivalAirportCode": arr_code, "airlineCode": AIRLINE_CODE})
                    route_flights.append(flight)
            else:
                logger.error(f"Failed to fetch data for {dep_code}->{arr_code} on {search_date} after retries.")
            time.sleep(0.5)
    ...
    def run(self):
        ...
        for dep, arr in routes_tn_de:
            all_scraped_flights.extend(self._scrape_route(dep, arr, is_eur_native=False, conversion_rate=conversion_rate))
        
        try:
            self.api_client.report_scraped_data(all_scraped_flights)
        except Exception as e:
            logger.critical(f"A fatal error occurred while reporting Tunisair data. Run aborted. Error: {e}")
            raise
